...
In the install command above, `<MATLAB_ROOT>` means the MATLAB installation root and is only used to install the Engine package. It is not a `config.json` field.
